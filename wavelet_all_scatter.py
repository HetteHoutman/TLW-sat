...
# lambda
xy_line = [min(df1[['lambda', 'lambda_min', 'lambda_max']].min(axis=None),
               df2[['lambda', 'lambda_min', 'lambda_max']].min(axis=None)),
           max(df1[['lambda', 'lambda_min', 'lambda_max']].max(axis=None),
               df2[['lambda', 'lambda_min', 'lambda_max']].max(axis=None))]
    ...
    try:
        ...
        if df1.loc[idx, 'theta_max'] < df1.loc[idx, 'theta']:
            plt.errorbar(df1.loc[idx, 'theta'] - 180, df2.loc[idx, 'theta'],
                         xerr=xerr, yerr=yerr, capsize=3, marker='s', c='k')
...
# from miscellaneous import k_spaced_lambda
#
# plot_result_lambda_hist(df1['lambda'], df2['lambda'], k_spaced_lambda([5, 35], 40)[0])
# plt.show()